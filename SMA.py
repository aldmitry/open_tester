...
class ForwardTester:
    def __init__(self, data_path, trade_params, params, tester, strategy,  transform_data, date_from, date_to = None,  max_max_drawdown = 80, min_sharp_rate = 0.1, min_n_trades = 1, min_mean_deal = 0, max_max_drawdown_duration = 1000000000, GO = 5000, n_processes = 10):
        """
        Помним что прогонщик обнуляет статистику перед каждым новым INSAMPLE
        :param data_path:
        :param trade_params:
        :param params:
        :param tester:
        ...
        :param date_from:
        :param date_to:
        :param max_max_drawdown:
        :param min_sharp_rate:
        :param min_n_trades:
        :param min_mean_deal:
        :param max_max_drawdown_duration:
        """
        self.GO = GO
        self.data_path = data_path
        self.params = params            # Параметры торговой стратегии
        trade_params = {i:list(trade_params[i]) for i in trade_params}
        self.trade_params = list(itertools.product(*trade_params.values()))
        self.trade_params = [dict(j) for j in [list(zip(trade_params.keys(), i)) for i in self.trade_params]]  # Параметры скользящего окна
        self.tester = tester
        self.strategy = strategy
        self.data_transform = transform_data
        self.date_from = date_from
        self.date_to = date_to
        ...
        self.min_n_trades = min_n_trades
        self.min_mean_deal = min_mean_deal
        self.max_max_drawdown_duration = max_max_drawdown_duration
        self.report = ForwardTestReport(trade_params, params)
        self.trades_table = pd.DataFrame()
        self.n_processes = n_processes

    def find_last_date_of_data(self):
        with open(self.data_path, mode='rb') as f:
            df = pickle.load(f)
            return df.index.iloc[-1]

    def make_begin_of_day(self, t0):
        return datetime.datetime(t0.year, t0.month, t0.day, 0, 0, 0)

    def make_end_trade_date(self, t0,trade_window):
        if trade_window == '1day':
            t1 = t0 + datetime.timedelta(days=1)
            return self.make_begin_of_day(t1)
        elif trade_window == '2day':
            t1 = t0 + datetime.timedelta(days=2)
            return self.make_begin_of_day(t1)
        elif trade_window == '3day':
            t1 = t0 + datetime.timedelta(days=3)
            return self.make_begin_of_day(t1)
        elif trade_window == '4day':
            t1 = t0 + datetime.timedelta(days=4)
            return self.make_begin_of_day(t1)
        elif trade_window == '5day':
            t1 = t0 + datetime.timedelta(days=5)
            return self.make_begin_of_day(t1)
        elif trade_window == '1week':
            start_week = t0.isocalendar()[1]
            while True:
                t0 = t0 + datetime.timedelta(days=1)
                if t0.isocalendar()[1] != start_week:
                    return self.make_begin_of_day(t0)
        elif trade_window == '2week':
            counter = 0
            start_week = t0.isocalendar()[1]
            while True:
                t0 = t0 + datetime.timedelta(days=1)
            ...
            while True:
                ...
                if t0.isocalendar()[1] != start_week:
                    ...
                    if counter == 4:
                        return self.make_begin_of_day(t0)
        elif trade_window == '1month':
            if t0.month == 12:
                return datetime.datetime(t0.year+1, 1, 1, 0, 0, 0)
            else:
                return datetime.datetime(t0.year, t0.month+1, 1, 0, 0, 0)
        elif trade_window == '2month':
            if t0.month+2 > 12:
                return datetime.datetime(t0.year+1, t0.month+2-12, 1, 0, 0, 0)
            else:
                return datetime.datetime(t0.year, t0.month+2, 1, 0, 0, 0)
        elif trade_window == '3month':
            if t0.month+3 > 12:
                return datetime.datetime(t0.year+1, t0.month+3-12, 1, 0, 0, 0)
            else:
                return datetime.datetime(t0.year, t0.month+3, 1, 0, 0, 0)
        elif trade_window == '4month':
            if t0.month+4 > 12:
                return datetime.datetime(t0.year+1, t0.month+4-12, 1, 0, 0, 0)
            else:
                return datetime.datetime(t0.year, t0.month+4, 1, 0, 0, 0)
        elif trade_window == '5month':
            if t0.month+2 > 12:
                return datetime.datetime(t0.year+1, t0.month+5-12, 1, 0, 0, 0)
            else:
                return datetime.datetime(t0.year, t0.month+5, 1, 0, 0, 0)
        elif trade_window == '6month':
            if t0.month+2 > 12:
                return datetime.datetime(t0.year+1, t0.month+5-12, 1, 0, 0, 0)
            else:
                return datetime.datetime(t0.year, t0.month+5, 1, 0, 0, 0)

    def forward_test(self, print_graph=True):
        data = self.data_transform(self.data_path)

        # Ищем границы прогона тестера
        start_date = self.make_begin_of_day(self.date_from)
        if self.date_to:
            end_date = self.date_to
        else:
            end_date = self.find_last_date_of_data()

        # Прогоняем тестер для каждой комбинации оптимизационных параметров
        for trade_params in self.trade_params:
            print()
            print('++++++++++++++++++++')
            print(trade_params)
            print('++++++++++++++++++++')
            print()
            tester = self.strategy()
            t_0_trade = start_date
            while True:
                print('Обучаем')
                # Проводим оптимизация по скользящему окну
                optimizer = self.tester(data = data, params = self.params, strategy = self.strategy, date_to = t_0_trade, learning_window = trade_params['learn_window'], n_processes = self.n_processes)
                optimizer.testing()
                best_window_params = optimizer.find_best_params()
                # print(optimizer.table_result().head())

                # Ищем интервал тестирования
                t_1_trade = self.make_end_trade_date(t_0_trade, trade_params['trade_window'])
                # Если в параметрам указано окончание тестирования больше чем в данных, ставим его по данным
                if t_1_trade > end_date:
                    t_1_trade = end_date

                print(f'Торгуем с {t_0_trade} по {t_1_trade}')

                # Тестируем на out sample
                print(best_window_params)
                tester.set_params(best_window_params)
                trade_df = data.resample_data(best_window_params['timeframe'])
                trade_df = data.make_indicators(trade_df, best_window_params)
                trade_df = data.cut_data(trade_df, date_from = t_0_trade, date_to = t_1_trade)
                tester.simulation(trade_df)

                # print('====== Результат =========')
                # print(tester.report.statistics())
                self.report.add_window_test_result(tester.report.statistics(), trade_params)

                # Таблица со всеми сделками
                if self.trades_table.shape[0] > 0:
                    self.trades_table = self.trades_table.append(tester.report.trades_table())
                else:
                    self.trades_table = tester.report.trades_table()

                # Изменяем даты тестирования
                t_0_trade = t_1_trade
                # Если пробежались по всем данным, выходим из цикла
                if t_0_trade >= end_date:
                    break

            if print_graph:
                df = data.cut_data(data.df, date_from=start_date, date_to=end_date)
                deals = self.trades_table
                StatReport.visualising_ekviti2(df,deals, trade_params)
...
if __name__ == '__main__':
    ...
    # print('Начало', datetime.datetime.now())
    # datapath = "D:\УИИ диплом\lion_tester\datas\SBRFF_1MIN.pickle"
    # params = {'fastma': range(5, 76, 5), 'slowma': range(55, 141, 5), 'min_hour_treshold': [9],
    #           'max_hour_treshold': [20], 'close_position_in_evening': [True], 'timeframe': ['10min'],
    #           'deal_period': [0]}
    # trade_params = {'learn_window': [18000, 24000, 36000, 48000, 60000, 72000], 'trade_window': ['1month']}
    # opt = ForwardTester(datapath, trade_params, params,
    #                     Tester, SimulatorDoubleMoovingAverage, DataTransformDoubleSMA,
    #                     date_from=datetime.datetime(2021, 7, 1, 0, 0, 0),
    #                     date_to=datetime.datetime(2022, 1, 1, 0, 0, 0),
    #                     n_processes=10)
    # opt.forward_test()
    # print(opt.report.result_table())
    # time_done = (time.time() - start_time)/60
    # print("--- %s minutes---" % time_done)
    datapath = "D:\УИИ диплом\lion_tester\datas\SBRFF_1MIN.pickle"
